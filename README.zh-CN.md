...
| 网络 Profile | `china`（**内置默认值**，国内优化镜像，覆盖选定下载场景）、`base`（仅官方源，需手动切换） |
...
| `base` | 仅使用官方源，不做镜像覆盖。适合海外或畅通网络环境。需手动设置 `SCIENCE_NETWORK_PROFILE=base` 启用。 |
...
| 网络 Profile | `china`（默认）、`base` | `china`（默认）、`base` | `china`（默认）、`base`、`enterprise`（私有镜像 / 内网终点） |
