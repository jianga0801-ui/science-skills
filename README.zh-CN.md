...
| 网络 Profile | `default`（官方源优先）、`china`（国内优化镜像，覆盖选定下载场景） |
...
| Profile | 作用 |
|---|---|
| `default` | 官方源优先，适合海外或畅通网络环境。 |
| `china` | 国内优化：API 查询仍走官方源，选定的大文件下载（如 PDB 坐标）走 PDBJ 等镜像；代理可见性诊断更严格。 |

`china` 是内置默认值，中国大陆用户开箱即用。
