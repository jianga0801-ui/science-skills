...
## 1. `normalize_pubmed_query` — Preview Chinese query normalization

Returns the English PubMed query that would be used for a Chinese natural
language request. This function does not access the network.
...
-   `query` (str, required) – Chinese or English user query
...
## 2. `search_pubmed` — Find PMIDs by query
...
Chinese queries are normalized to an English PubMed query before calling
ESearch.
...
## 3. `global_database_discovery` — Count hits across all NCBI databases
